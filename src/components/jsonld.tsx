...
let POSTS: Post[] = [
  {
    id: '1',
    slug: 'welcome-to-ai-forum',
    title: 'Welcome to the AI Forum MVP',
    body: 'This is a minimal, crawlable forum. Create a post, add comments, and ship.',
    author: 'System',
    createdAt: Date.now(),
  },
]
...
export async function createPost(input: { title: string; body: string; author?: string }) {
  const slug = `${slugify(input.title, { lower: true, strict: true })}-${Math.random()
    .toString(36)
    .slice(2, 6)}`
  const post: Post = {
